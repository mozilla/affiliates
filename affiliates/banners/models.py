...
class Category(CachingMixin, MPTTModel):
    ...
    def links(self):
        ...
        content_types = ContentType.objects.get_for_models(
            ImageBannerVariation, TextBannerVariation, FirefoxUpgradeBannerVariation)

        # Filter links by variation id and content type.
        variation_filters = []
        for variation_class, content_type in content_types.items():
            ids = variation_class.objects.filter(banner__category=self).values_list('id', flat=True)
            variation_filters.append(Q(banner_variation_id__in=ids,
    ...
    def link_clicks(self):
        ...
        """
        return sum(link.link_clicks for link in self.links)
