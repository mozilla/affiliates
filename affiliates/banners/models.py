...
class CategoryQuerySet(CachingQuerySet):
    def with_visible_banners(self):
        """Filter out categories that don't have any visible banners."""
        return self.filter(Q(imagebanner__visible=True) | Q(textbanner__visible=True) |
                           Q(firefoxupgradebanner__visible=True))


# MPTT and django-cache-machine, living in harmony!
class CategoryManager(TreeManager, CachingManager):
    def get_query_set(self):
        return CategoryQuerySet(self.model, using=self._db)

    def with_visible_banners(self):
        return self.all().with_visible_banners()
...
class Category(CachingMixin, MPTTModel):
    ...
    def links(self, prefetch=None, **filters):
        ...
        category.
        """
        # Bail early if there are no banners in a category.
